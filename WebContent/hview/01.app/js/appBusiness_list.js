layui.use(['form', 'table'], function() {
	...
	var listTable = table.render({
		id: 'listTable',
		elem: '#tableContent',
		url: '/piPaiCampus/rest/appBusiness/queryAppBusiness',
		method: 'post',
		cols: [
			[{
				type: 'numbers',
				...
				templet: function(d) {
					...
					if(a == 0) {
					//	return '停止'
						return '<span style="color:#FF5722;">停止</span>'
					}
					if(a == 1) {
						//return '启动'
						return '<span style="color:#009688;">启动</span>'
					}
				}
			}, {
				field: '',
	...
	function editItem(data) {
		...
		layer.open({
			...
			title: '编辑',
			area: ['700px', '600px'],
			content: ['appBusiness_edit.html?randmStr=' + randmStr, 'no'],
			id: 'userEdit',
	...
	function deleteItem(data) {
		...
		$.ajax({
			url: "/piPaiCampus/rest/appBusiness/deleteAppBusiness",
			type: 'POST',
			dataType: 'text',
			data: 'para=' + fstr,
	...
	$("#btnAdd").click(function() {
		...
		layer.open({
			...
			maxmin: true,
			area: ['748px', '712px'],
			content: ['appBusiness_add.html?randmStr=' + randmStr, 'no'],
			id: 'userAdd',
			success: function(layero, index) {
				layer.setTop(layero); //重点2
			//	layer.iframeAuto(index);
