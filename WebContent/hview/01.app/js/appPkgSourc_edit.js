...
layui.use(['form', 'layedit',], function () {
   ...
   /* //自定义验证规则
    ...
    form.on('submit(doFormSub)', function (data) {
        ...
        $.ajax({
            url: "/piPaiCampus/rest/appMgr/updatePkgSourc",
            type: 'POST',
            dataType: 'text',
            data: 'para=' + fstr,
    ...
    $(function(){
        ...
        $("input[name='funcResrName']").val(initData.funcResrName);
        $("input[name='versionId']").val(initData.versionId);
        $("input[name='funcResrCode']").val(initData.funcResrCode);
        $("input[name='pkgUrl']").val(initData.pkgUrl);
        $("input[name='funUrl']").val(initData.funUrl);
        $("#vsnremark").val(initData.remark); 
        $('input:radio[name=userType]').each(function (index, el) {
            // console.log(el);
            el.checked = false;
            if ($(el).val() == initData.userType)
                el.checked = true;
        });
