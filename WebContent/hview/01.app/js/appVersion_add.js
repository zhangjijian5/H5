...
layui.use(['form', 'layedit', 'upload'], function() {
	...
	$("#vCode").blur(function() {
		var fstr = '';
		fstr += 'versionCode=' + $("#vCode").val();
		$.ajax({
			url: "/piPaiCampus/rest/appMgr/checkVersionCode",
			type: 'POST',
			dataType: 'text',
			data: 'para=' + fstr,
	...
	form.on('submit(fsubmit)', function(data) {
		...
		$.ajax({
			url: "/piPaiCampus/rest/appMgr/addAppVersion",
			type: 'POST',
			dataType: 'text',
			data: 'para=' + fstr,
	...
	$("#vRemark").blur(function() {
		var vRemark = $("#vRemark").val();
		var len = vRemark.length;
		if(len > 50) {
			top.layer.msg("内容不合法");
			$("#vRemark").val("");
			$("#vRemark").focus();
