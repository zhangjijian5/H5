...
layui.use(['form', 'layedit',], function () {
    ...
    form.on('submit(doFormSub)', function (data) {
        ...
        $.ajax({
            url: "/piPaiCampus/rest/campusBs/setExpressRate",
            type: 'POST',
            dataType: 'text',
            data: 'para=' + fstr,
    ...
    $(function(){
        ...
        $("input[name='expressBrandCode']").val(initData.expressBrandCode);
        $("input[name='distributionFeeRate']").val(initData.distributionFeeRate);
        $("input[name='feeBusiness']").val(initData.feeBusiness);
        $("input[name='disFeeLowCost']").val(initData.disFeeLowCost);
        $('input:radio[name=serviceType]').each(function (index, el) {
            // console.log(el);
            el.checked = false;
            if ($(el).val() == initData.serviceType)
                el.checked = true;
        });
        form.render('radio');
    });
