...
layui.use(['form', 'layedit','upload'], function () {
   ...
   /* var uploadInst = upload.render({
        ...
        exts: 'zip|rar|7z',
            ...
            //   console.log(res);
            if (res.code == 0) {
                $("input[name='versionUrl']").val(res.data.savePath);
            }
    ...
    form.on('submit(doFormSub)', function (data) {
        ...
        // console.log(fstr);
        $.ajax({
            url: "/piPaiCampus/rest/distributionMgr/addDistributionEmployee",
            type: 'POST',
            dataType: 'text',
            data: 'para=' + fstr,
   ...
   $(function() {
		...
		$.ajax({
			url: "/piPaiCampus/rest/shopMgr/queryOperationStore",
			type: 'POST',
			dataType: 'text',
			data: 'para=' + fstr,
			success: function(result) {
				...
				if(data != null) {
					for(var i = 0; i < results.data.length; i++) {
						var item = data[i];
						$("input[name='ssOrgId']").val(item.id);
						$("input[name='ssName']").val(item.ssName);
					}
				}
			}
		});
