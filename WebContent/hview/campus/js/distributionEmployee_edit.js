...
layui.use(['form', 'layedit',], function () {
    ...
    $(function(){
        ...
        $("input[name='employeeId']").val(initData.employeeId);
        $("input[name='ssName']").val(initData.ssName);
        $("input[name='ssType']").val(initData.ssType);
        $("input[name='employeeName']").val(initData.employeeName);
      	$("input[name='telephone']").val(initData.telephone);
        $('input:radio[name=deltag]').each(function (index, el) {
            // console.log(el);
            el.checked = false;
            if ($(el).val() == initData.deltag)
                el.checked = true;
        });
        form.render('radio');
		$("#od").val(initData.onDuty); 
		$("#yrm").val(initData.yesReceiveMes); 
		$("#yj").val(initData.yesJob); 
