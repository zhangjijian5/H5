...
layui.use(['form', 'layedit',], function () {
   ...
   /* //自定义验证规则
    ...
    form.on('submit(doFormSub)', function (data) {
        ...
        // console.log(fstr);
        $.ajax({
            url: "/piPaiCampus/rest/zone/changeZone",
            type: 'POST',
            dataType: 'text',
            data: 'para=' + fstr,
    ...
    $(function(){
        console.log(initData);
		 $("input[name='zoneName']").val(initData.zoneName);
        $("input[name='campusId']").val(initData.campusId);
        $("input[name='zoneLevel']").val(initData.zoneLevel);
        $("input[name='zoneType']").val(initData.zoneType);
        $("input[name='id']").val(initData.id);
        $("input[name='parentId']").val(initData.parentId);
