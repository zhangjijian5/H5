...
//		$("#a_dept").attr("href", $("#a_dept").attr("href") + "?ss=" + Math.random());
	...
	if(typeof(accountJson) == undefined || accountJson == null || !account)
		location.href = 'login.html';
	console.log("登入主页面");*/
	...
	/*function initMenu() {
		...
		/*$("#navBar").html("");
		 var optionStr="546546512132132132";			
		$("#navBar").append(optionStr);*/
	/*	var account = sessionStorage.getItem("accountInfo");
		console.log(account);
		var accountJson = JSON.parse(account);
		...
		var menuTwo = accountJson.secondMenus;
		for(var i = 0; i < accountJson.menus.length; i++) {
			...
			if(data[i].parentId == 0) { //取出父元素的菜单，拼进页面
				...
				for(var j = 0; j < menuTwo.length; j++) { //遍历二级菜单
					var tempMenu = menuTwo[j];
					for(var a = 0; a < tempMenu.length; a++) {
						if(tempMenu[a].parentId == data[i].menuId) {
							menu += "<dl class='layui-nav-child'>"
							menu += "<dd>"
							menu += "<a href='" + tempMenu[a].href + "' target='mcontent'>" + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + tempMenu[a].title2 + "</a>"
							menu += "</dd>"
							menu += "</dl>"
						}
					}
