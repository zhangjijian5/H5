...
*/
	...
	form.on('submit(fsubmit)', function(data) {
		...
		$.ajax({
			url: "/piPaiCampus/rest/school/addCampus",
			type: 'POST',
			dataType: 'text',
			data: 'para=' + fstr,
	...
	$("#cName").blur(function() {
		var fstr = '';
		fstr += 'campusName=' + $("#cName").val();
		$.ajax({
			url: "/piPaiCampus/rest/campusMgr/checkCampus",
			type: 'POST',
			dataType: 'text',
			data: 'para=' + fstr,
	...
	$(function() {
		...
		$("input[name='schoolId']").val(initData.id);
		/*  $("input[name='areaCode']").val(initData.areaCode);
		  $("input[name='schoolCode']").val(initData.schoolCode);
