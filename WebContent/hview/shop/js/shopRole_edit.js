...
*/
    ...
    form.on('submit(doFormSub)', function (data) {
        ...
        $.ajax({
            url: "/piPaiCampus/rest/shopMgr/updateShopRole",
            type: 'POST',
            dataType: 'text',
            data: 'para=' + fstr,
    ...
    $(function(){
        ...
        $("input[name='powerCode']").val(initData.powerCode);
        $("input[name='powerName']").val(initData.powerName);
        ...
        $('input:radio[name=ssOrgType]').each(function (index, el) {
            // console.log(el);
            el.checked = false;
            if ($(el).val() == initData.ssOrgType)
                el.checked = true;
        });
        form.render('radio');
